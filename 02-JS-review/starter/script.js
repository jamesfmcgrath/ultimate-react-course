...
async function getTodos(user, done = false) {
  try {
    ...
    console.log(data);
    const todos = data.filter((todo) => todo.userId === user && todo.completed === done).map((todo) => todo.title);
    const onlyCompleted = data.reduce((acc, todo) => (todo.completed ? acc + 1 : acc), 0);
